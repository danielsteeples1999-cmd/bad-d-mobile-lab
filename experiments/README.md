...
| [EXP-010](EXP-010/README.md) | First priority-queue-driven session: does bulk-media-intake survive interruption, and does memory grow across repeated cycles? | **Interruption:** clean total loss on reload (no persistence exists), tool remains fully usable after — not a P0, reclassified as a feature gap. **Memory:** looked like a leak with unforced GC (backing storage 5.4MB→58.8MB over 5 cycles), but forced GC (`HeapProfiler.collectGarbage`) showed a flat 0.03MB floor every cycle — **not a leak, was unswept garbage.** | Both P1 unknowns resolved with evidence; see `PRIORITY_QUEUE.md` for what moved where |
| [EXP-011](EXP-011/README.md) | CONC-REPRO-001: does EXP-009's n=1 concurrency conclusion (optimal 2–4×, flat by 8×) actually reproduce? | 2×→4× **REPRODUCED** (consistent ranking across all 3 runs each, thin 6ms margin at the closest pair — reported honestly, not oversold). 4×→8× **CONFIRMED flat** (overlapping ranges). New finding only visible with replication: concurrency also sharply reduces run-to-run variance (2× stdev 225.7ms vs 4×/8× ~25–39ms). Recommendation: default concurrency 4. | `REPRODUCED` — first item to complete the full executable-priority-queue loop |
| [EXP-012](EXP-012/README.md) | Built the reusable Fixture Acceptance Gate (`tools/fixture-acceptance/`, `EXPERIMENT_PROTOCOL.md`), self-tested it, then used it to gate HASH-NEAR-001. | Self-test: deliberately bad fixture correctly `FIXTURE_REJECTED` (CONTENT_VERIFICATION caught the mismatch); genuine near-duplicate correctly `FIXTURE_ACCEPTED` (all 10 criteria, incl. `NO_SILENT_MUTATION`). Only then interpreted HASH-NEAR-001: `sha256` differs, `audioContentHash` matches — **SUPPORTED** for the container-difference case specifically. | Reusable gate `CANDIDATE`; HASH-NEAR-001 `MEASURED` (narrow, correctly scoped claim) |
